...
class TestUtils(unittest.TestCase):
    def test_lazy_call(self):
        with mock.patch("boto3.utils.import_module") as importer:
            ...
            lazy_function = utils.lazy_call("fakemodule.FakeModule.entry_point")
...
class TestLazyLoadedWaiterModel(unittest.TestCase):
    def test_get_waiter_model_is_lazy(self):
        session = mock.Mock()
        waiter_model = utils.LazyLoadedWaiterModel(session, "myservice", "2014-01-01")
        ...
        session.get_waiter_model.return_value.get_waiter.assert_called_with("Foo")
