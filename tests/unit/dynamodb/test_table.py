...
class BaseTransformationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.flush_amount = 2
        self.batch_writer = BatchWriter(self.table_name, self.client, self.flush_amount)

    def assert_batch_write_calls_are(self, expected_batch_writes):
        self.assertEqual(self.client.batch_write_item.call_count, len(expected_batch_writes))
        batch_write_calls = [args[1] for args in self.client.batch_write_item.call_args_list]
    ...
    def test_unprocessed_items_added_to_next_batch(self):
        self.client.batch_write_item.side_effect = [
            {"UnprocessedItems": {self.table_name: [{"PutRequest": {"Item": {"Hash": "foo2"}}}],},},
    ...
    def test_all_items_flushed_on_exit(self):
        ...
        self.assert_batch_write_calls_are(
            [{"RequestItems": {self.table_name: [{"PutRequest": {"Item": {"Hash": "foo1"}}},]},},]
        )
    ...
    def test_never_send_more_than_max_batch_size(self):
        ...
        self.client.batch_write_item.side_effect = [
            ...
            {
                "UnprocessedItems": {
                    self.table_name: [{"PutRequest": {"Item": {"Hash": "foo2"}}},],
                },
            },
            {"UnprocessedItems": {}},
        ]
        ...
        self.assert_batch_write_calls_are([first_batch, second_batch, third_batch])
    ...
    def test_repeated_flushing_on_exit(self):
        ...
        self.client.batch_write_item.side_effect = [
            ...
            {
                "UnprocessedItems": {
                    self.table_name: [{"PutRequest": {"Item": {"Hash": "foo3"}}},],
                },
            },
            {"UnprocessedItems": {}},
        ]
        ...
        # we need to send another batch.
        third_batch = {
            "RequestItems": {self.table_name: [{"PutRequest": {"Item": {"Hash": "foo3"}}},]}
        }
        self.assert_batch_write_calls_are([first_batch, second_batch, third_batch])

    def test_auto_dedup_for_dup_requests(self):
        with BatchWriter(
            self.table_name, self.client, flush_amount=5, overwrite_by_pkeys=["pkey", "skey"],
        ) as b:
            # dup 1
            b.put_item(Item={"pkey": "foo1", "skey": "bar1", "other": "other1"})
            b.put_item(Item={"pkey": "foo1", "skey": "bar1", "other": "other2"})
            # dup 2
            b.delete_item(
                Key={"pkey": "foo1", "skey": "bar2",}
            )
            b.put_item(Item={"pkey": "foo1", "skey": "bar2", "other": "other3"})
            # dup 3
            b.put_item(Item={"pkey": "foo2", "skey": "bar2", "other": "other3"})
            b.delete_item(
                Key={"pkey": "foo2", "skey": "bar2",}
            )
            # dup 4
            b.delete_item(
                Key={"pkey": "foo2", "skey": "bar3",}
            )
            b.delete_item(
                Key={"pkey": "foo2", "skey": "bar3",}
            )
        ...
        first_batch = {
            "RequestItems": {
                self.table_name: [
                    {"PutRequest": {"Item": {"pkey": "foo1", "skey": "bar1", "other": "other2"}}},
                    {"PutRequest": {"Item": {"pkey": "foo1", "skey": "bar2", "other": "other3"}}},
                    {"DeleteRequest": {"Key": {"pkey": "foo2", "skey": "bar2",}}},
                    {"DeleteRequest": {"Key": {"pkey": "foo2", "skey": "bar3",}}},
                    {"DeleteRequest": {"Key": {"pkey": "foo3", "skey": "bar3",}}},
                ]
            }
        }
        second_batch = {
