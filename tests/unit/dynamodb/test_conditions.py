...
class TestK(unittest.TestCase):
    ...
    def test_and(self):
        with self.assertRaisesRegex(DynamoDBOperationNotSupportedError, "AND"):
            self.attr & self.attr2

    def test_or(self):
        with self.assertRaisesRegex(DynamoDBOperationNotSupportedError, "OR"):
            self.attr | self.attr2

    def test_not(self):
        with self.assertRaisesRegex(DynamoDBOperationNotSupportedError, "NOT"):
            ~self.attr
    ...
    def test_eq_inequality(self):
        attr_copy = copy.deepcopy(self.attr)
        self.assertNotEqual(self.attr.eq(self.value), attr_copy.eq(self.value2))
...
class TestConditions(unittest.TestCase):
    ...
    def test_eq(self):
        self.build_and_assert_expression(
            ...
            {"format": "{0} {operator} {1}", "operator": "=", "values": (self.value, self.value2),},
        )
    ...
    def test_ae(self):
        self.build_and_assert_expression(
            ...
            {"format": "{operator}({0})", "operator": "attribute_exists", "values": (self.value,),},
        )
    ...
    def test_size(self):
        self.build_and_assert_expression(
            Size(self.value),
            {"format": "{operator}({0})", "operator": "size", "values": (self.value,)},
        )
    ...
    def test_size_can_use_attr_methods(self):
        ...
        self.build_and_assert_expression(
            size.eq(self.value),
            {"format": "{0} {operator} {1}", "operator": "=", "values": (size, self.value),},
        )
    ...
    def test_size_can_use_and(self):
        ...
        self.build_and_assert_expression(
            size & ae, {"format": "({0} {operator} {1})", "operator": "AND", "values": (size, ae)},
        )
    ...
    def test_and(self):
        ...
        self.build_and_assert_expression(
            and_cond,
            {"format": "({0} {operator} {1})", "operator": "AND", "values": (cond1, cond2),},
        )
    ...
    def test_or(self):
        ...
        self.build_and_assert_expression(
            or_cond,
            {"format": "({0} {operator} {1})", "operator": "OR", "values": (cond1, cond2),},
        )
    ...
    def test_not(self):
        ...
        not_cond = Not(cond)
        self.build_and_assert_expression(
            not_cond, {"format": "({operator} {0})", "operator": "NOT", "values": (cond,)},
        )
...
class TestConditionExpressionBuilder(unittest.TestCase):
    ...
    def assert_condition_expression_build(
        self, condition, ref_string, ref_names, ref_values, is_key_condition=False
    ):
    ...
    def test_build_attribute_with_attr_value(self):
        ...
        self.assert_condition_expression_build(
            a.eq(value), "#n0 = #n1", {"#n0": "myattr", "#n1": "myreference"}, {}
        )
