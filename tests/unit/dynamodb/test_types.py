...
class TestSerializer(unittest.TestCase):
    ...
    def test_serialize_decimal(self):
        self.assertEqual(self.serializer.serialize(Decimal("1.25")), {"N": "1.25"})
    ...
    def test_serialize_NaN_error(self):
        with self.assertRaisesRegex(TypeError, "Infinity and NaN not supported"):
    ...
    def test_serialize_binary(self):
        self.assertEqual(self.serializer.serialize(Binary(b"\x01")), {"B": b"\x01"})

    def test_serialize_bytearray(self):
        self.assertEqual(self.serializer.serialize(bytearray([1])), {"B": b"\x01"})
    ...
    def test_serialize_binary_set(self):
        serialized_value = self.serializer.serialize(set([Binary(b"\x01"), Binary(b"\x02")]))
        self.assertEqual(len(serialized_value), 1)
    ...
    def test_serialize_list(self):
        ...
        self.assertCountEqual(
            serialized_value["L"], [{"S": "foo"}, {"N": "1"}, {"L": [{"N": "1"}]}]
        )

    def test_serialize_map(self):
        serialized_value = self.serializer.serialize({"foo": "bar", "baz": {"biz": 1}})
        self.assertEqual(
            serialized_value, {"M": {"foo": {"S": "bar"}, "baz": {"M": {"biz": {"N": "1"}}}}},
        )
...
class TestDeserializer(unittest.TestCase):
    ...
    def test_deserialize_integer(self):
        self.assertEqual(self.deserializer.deserialize({"N": "1"}), Decimal("1"))

    def test_deserialize_decimal(self):
        self.assertEqual(self.deserializer.deserialize({"N": "1.25"}), Decimal("1.25"))

    def test_deserialize_string(self):
        self.assertEqual(self.deserializer.deserialize({"S": "foo"}), "foo")

    def test_deserialize_binary(self):
        self.assertEqual(self.deserializer.deserialize({"B": b"\x00"}), Binary(b"\x00"))
    ...
    def test_deserialize_string_set(self):
        self.assertEqual(self.deserializer.deserialize({"SS": ["foo", "bar"]}), set(["foo", "bar"]))
