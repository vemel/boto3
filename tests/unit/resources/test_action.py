...
class TestServiceActionCall(BaseTestCase):
    ...
    def test_service_action_creates_params(self, params_mock):
        ...
        self.assertTrue(params_mock.called, "Parameters for operation not created")
    ...
    def test_service_action_calls_operation(self, params_mock):
        ...
        self.assertEqual(response, "response", "Unexpected low-level response data returned")
    ...
    def test_service_action_calls_resource_handler(self, handler_mock, params_mock):
        ...
        action = ServiceAction(
            action_model=action_model, factory=factory, service_context=service_context
        )
...
class TestWaiterActionCall(BaseTestCase):
    def setUp(self):
        ...
        self.waiter_def = {
            ...
            "params": [{"target": "Frob", "sourceType": "identifier", "source": "Name"}],
        }
...
class TestBatchActionCall(BaseTestCase):
    ...
    def test_batch_action_creates_parameters_from_items(self):
        self.action_def["request"]["params"] = [
            ...
            {"target": "Delete.Objects[].Key", "source": "data", "path": "Key"},
        ]
        ...
        item1.meta = ResourceMeta(
            "test", client=client, data={"BucketName": "bucket", "Key": "item1"}
        )

        item2 = mock.Mock()
        item2.meta = ResourceMeta(
            "test", client=client, data={"BucketName": "bucket", "Key": "item2"}
        )
        ...
        client.get_frobs.assert_called_with(
            Bucket="bucket", Delete={"Objects": [{"Key": "item1"}, {"Key": "item2"}]}
        )
