...
class TestServiceActionParams(BaseTestCase):
    def test_service_action_params_identifier(self):
        request_model = Request(
            {
                "operation": "GetFrobs",
                "params": [{"target": "WarehouseUrl", "source": "identifier", "name": "Url"}],
            }
        )
    ...
    def test_service_action_params_data_member(self):
        request_model = Request(
            {
                "operation": "GetFrobs",
                "params": [{"target": "WarehouseUrl", "source": "data", "path": "SomeMember"}],
            }
        )
    ...
    def test_service_action_params_constants(self):
        request_model = Request(
            {
                "operation": "GetFrobs",
                "params": [
                    {"target": "Param1", "source": "string", "value": "param1"},
        ...
        self.assertEqual(params["Param1"], "param1", "Parameter not set from string constant")
        self.assertEqual(params["Param2"], 123, "Parameter not set from integer constant")
        self.assertEqual(params["Param3"], True, "Parameter not set from boolean constant")

    def test_service_action_params_input(self):
        request_model = Request(
            {"operation": "GetFrobs", "params": [{"target": "Param1", "source": "input"}],}
        )
    ...
    def test_service_action_params_invalid(self):
        request_model = Request(
            {"operation": "GetFrobs", "params": [{"target": "Param1", "source": "invalid"}],}
        )
    ...
    def test_service_action_params_list(self):
        request_model = Request(
            {
                "operation": "GetFrobs",
                "params": [{"target": "WarehouseUrls[0]", "source": "string", "value": "w-url"}],
            }
        )

        params = create_request_parameters(None, request_model)

        self.assertIsInstance(params["WarehouseUrls"], list, "Parameter did not create a list")
        self.assertEqual(
            len(params["WarehouseUrls"]), 1, "Parameter list should only have a single item",
        )
        self.assertIn("w-url", params["WarehouseUrls"], "Parameter not in expected list")

    def test_service_action_params_reuse(self):
        request_model = Request(
            {
                "operation": "GetFrobs",
                "params": [{"target": "Delete.Objects[].Key", "source": "data", "path": "Key"}],
            }
        )
        ...
        self.assertEqual(params, {"Delete": {"Objects": [{"Key": "item1"}, {"Key": "item2"}]}})
...
class TestStructBuilder(BaseTestCase):
    ...
    def test_provided_index_with_wildcard(self):
        ...
        self.assertEqual(params["foo"], [{"bar": 123, "baz": 456}, {"bar": 789, "baz": 123}])
