...
class TestInstanceDeleteTags(unittest.TestCase):
    ...
    def test_delete_tags_injected(self):
        self.assertTrue(
            hasattr(self.instance_resource, "delete_tags"),
    ...
    def test_mutating_filters(self):
        ...
        running_instances = self.service_resource.instances.filter(Filters=instance_filters)

        # This should not impact the already-created filter.
        instance_filters.append({"Name": "instance-type", "Values": ["c4.large"]})

        stubber.add_response(
            method="describe_instances",
            service_response={"Reservations": []},
            expected_params={"Filters": [{"Name": "instance-state-name", "Values": ["running"]}]},
        )
