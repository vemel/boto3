...
class TestS3MethodInjection(unittest.TestCase):
    def test_transfer_methods_injected_to_client(self):
        ...
        self.assertTrue(
            hasattr(client, "upload_file"), "upload_file was not injected onto S3 client",
        )
        self.assertTrue(
            hasattr(client, "download_file"), "download_file was not injected onto S3 client",
        )
        self.assertTrue(hasattr(client, "copy"), "copy was not injected onto S3 client")

    def test_bucket_resource_has_load_method(self):
        session = boto3.session.Session(region_name="us-west-2")
        bucket = session.resource("s3").Bucket("fakebucket")
        self.assertTrue(hasattr(bucket, "load"), "load() was not injected onto S3 Bucket resource.")

    def test_transfer_methods_injected_to_bucket(self):
        bucket = boto3.resource("s3").Bucket("my_bucket")
        self.assertTrue(
            hasattr(bucket, "upload_file"), "upload_file was not injected onto S3 bucket",
        )
        self.assertTrue(
            hasattr(bucket, "download_file"), "download_file was not injected onto S3 bucket",
        )
        self.assertTrue(hasattr(bucket, "copy"), "copy was not injected onto S3 bucket")

    def test_transfer_methods_injected_to_object(self):
        obj = boto3.resource("s3").Object("my_bucket", "my_key")
        self.assertTrue(hasattr(obj, "upload_file"), "upload_file was not injected onto S3 object")
        self.assertTrue(
            hasattr(obj, "download_file"), "download_file was not injected onto S3 object",
        )
        self.assertTrue(hasattr(obj, "copy"), "copy was not injected onto S3 object")


class BaseTransferTest(unittest.TestCase):
    def setUp(self):
        self.session = boto3.session.Session(
            aws_access_key_id="foo", aws_secret_access_key="bar", region_name="us-west-2",
        )
    ...
    def stub_head(self, content_length=4, expected_params=None):
        ...
        self.stubber.add_response(
            method="head_object", service_response=head_response, expected_params=expected_params,
        )
...
class TestCopy(BaseTransferTest):
    ...
    def stub_multipart_copy(self, part_size, num_parts):
        ...
        self.stub_head(content_length=total_size, expected_params=self.copy_source)
        ...
        for i in range(num_parts):
            # Fill in the parts
            part_number = i + 1
            copy_range = "bytes=%s-%s" % (i * part_size, i * part_size + (part_size - 1),)
    ...
    def test_client_copy(self):
        ...
        with self.stubber:
            response = self.s3.meta.client.copy(self.copy_source, self.bucket, self.key)
    ...
    def test_copy_progress(self):
        ...
        transfer_config = TransferConfig(
            multipart_chunksize=chunksize, multipart_threshold=1, max_concurrency=1
        )
    ...
    def test_raises_value_error_on_invalid_fileobj(self):
        with self.stubber:
            with self.assertRaises(ValueError):
                self.s3.meta.client.upload_fileobj(Fileobj="foo", Bucket=self.bucket, Key=self.key)
...
class TestDownloadFileobj(BaseTransferTest):
    ...
    def stub_get_object(self, full_contents, start_byte=0, end_byte=None):
        ...
        if end_byte is not None:
            ...
            content_range = "bytes=%s-%s/%s" % (start_byte, end_byte, len(full_contents),)
    ...
    def stub_multipart_download(self, contents, part_size, num_parts):
        ...
        for i in range(num_parts):
            ...
            self.stub_get_object(full_contents=contents, start_byte=start_byte, end_byte=end_byte)
    ...
    def test_multipart_download(self):
        ...
        self.stub_multipart_download(contents=self.contents, part_size=5, num_parts=11)
        transfer_config = TransferConfig(
        ...
        with self.stubber:
            self.s3.meta.client.download_fileobj(
                Bucket=self.bucket, Key=self.key, Fileobj=self.fileobj, Config=transfer_config,
            )
...
class TestServiceResource(unittest.TestCase):
    ...
    def test_unsigned_signature_version_is_not_corrupted(self):
        ...
        self.assertIs(resource.meta.client.meta.config.signature_version, botocore.UNSIGNED)
