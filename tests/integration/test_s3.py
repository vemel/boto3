...
def assert_files_equal(first, second):
    ...
    if first_md5 != second_md5:
        raise AssertionError(
            "Files are not equal: %s(md5=%s) != %s(md5=%s)" % (first, first_md5, second, second_md5)
        )
...
def setup_module():
    ...
    params = {
        ...
        "CreateBucketConfiguration": {"LocationConstraint": _DEFAULT_REGION,},
    }
    try:
...
def clear_out_bucket(bucket, region, delete_bucket=False):
    ...
    if delete_bucket:
        try:
            ...
            # to be deleted.
            LOG.debug("delete_bucket() raised an exception: %s", e, exc_info=True)
...
class TestS3Transfers(unittest.TestCase):
    ...
    def wait_until_object_exists(self, key_name, extra_params=None, min_successes=3):
        waiter = self.client.get_waiter("object_exists")
    ...
    def assert_has_public_read_acl(self, response):
        ...
        public_read = [g["Grantee"].get("URI", "") for g in grants if g["Permission"] == "READ"]
        self.assertIn("groups/global/AllUsers", public_read[0])

    def test_copy(self):
        self.client.put_object(Bucket=self.bucket_name, Key="foo", Body="beach")
    ...
    def test_upload_fileobj(self):
        ...
        self.client.upload_fileobj(Fileobj=fileobj, Bucket=self.bucket_name, Key="foo")
        self.addCleanup(self.delete_object, "foo")
    ...
    def test_upload_fileobj_progress(self):
        ...
        config = boto3.s3.transfer.TransferConfig(
            multipart_chunksize=chunksize, multipart_threshold=chunksize, max_concurrency=1,
        )
    ...
    def test_download_fileobj(self):
        fileobj = BytesIO()
        self.client.put_object(Bucket=self.bucket_name, Key="foo", Body=b"beach")
        self.addCleanup(self.delete_object, "foo")

        self.wait_until_object_exists("foo")
        self.client.download_fileobj(Bucket=self.bucket_name, Key="foo", Fileobj=fileobj)

        self.assertEqual(fileobj.getvalue(), b"beach")

    def test_upload_below_threshold(self):
        config = boto3.s3.transfer.TransferConfig(multipart_threshold=2 * 1024 * 1024)
        transfer = self.create_s3_transfer(config)
        filename = self.files.create_file_with_size("foo.txt", filesize=1024 * 1024)
    ...
    def test_upload_above_threshold(self):
        config = boto3.s3.transfer.TransferConfig(multipart_threshold=2 * 1024 * 1024)
        transfer = self.create_s3_transfer(config)
        filename = self.files.create_file_with_size("20mb.txt", filesize=20 * 1024 * 1024)
    ...
    def test_upload_file_above_threshold_with_acl(self):
        ...
        transfer = self.create_s3_transfer(config)
        filename = self.files.create_file_with_size("6mb.txt", filesize=6 * 1024 * 1024)
        extra_args = {"ACL": "public-read"}
        transfer.upload_file(filename, self.bucket_name, "6mb.txt", extra_args=extra_args)
        self.addCleanup(self.delete_object, "6mb.txt")

        self.assertTrue(self.object_exists("6mb.txt"))
        response = self.client.get_object_acl(Bucket=self.bucket_name, Key="6mb.txt")
    ...
    def test_upload_file_above_threshold_with_ssec(self):
        ...
        response = self.client.head_object(Bucket=self.bucket_name, Key="6mb.txt", **extra_args)
    ...
    def test_progress_callback_on_upload(self):
        ...
        filename = self.files.create_file_with_size("20mb.txt", filesize=20 * 1024 * 1024)
        transfer.upload_file(filename, self.bucket_name, "20mb.txt", callback=progress_callback)
        self.addCleanup(self.delete_object, "20mb.txt")
    ...
    def test_callback_called_once_with_sigv4(self):
        ...
        client = self.session.client("s3", self.region, config=Config(signature_version="s3v4"))
        transfer = boto3.s3.transfer.S3Transfer(client)
        filename = self.files.create_file_with_size("10mb.txt", filesize=10 * 1024 * 1024)
        transfer.upload_file(filename, self.bucket_name, "10mb.txt", callback=progress_callback)
    ...
    def test_can_send_extra_params_on_upload(self):
        ...
        transfer.upload_file(
            filename, self.bucket_name, "foo.txt", extra_args={"ACL": "public-read"}
        )
        self.addCleanup(self.delete_object, "foo.txt")

        response = self.client.get_object_acl(Bucket=self.bucket_name, Key="foo.txt")
        self.assert_has_public_read_acl(response)

    def test_can_configure_threshold(self):
        config = boto3.s3.transfer.TransferConfig(multipart_threshold=6 * 1024 * 1024)
        transfer = self.create_s3_transfer(config)
        filename = self.files.create_file_with_size("foo.txt", filesize=8 * 1024 * 1024)
        transfer.upload_file(filename, self.bucket_name, "foo.txt")
        self.addCleanup(self.delete_object, "foo.txt")
    ...
    def test_can_send_extra_params_on_download(self):
        ...
        self.client.put_object(
            Bucket=self.bucket_name, Key="foo.txt", Body=b"hello world", **extra_args
        )
        self.addCleanup(self.delete_object, "foo.txt")
        transfer = self.create_s3_transfer()
        ...
        transfer.download_file(self.bucket_name, "foo.txt", download_path, extra_args=extra_args)
    ...
    def test_progress_callback_on_download(self):
        ...
        filename = self.files.create_file_with_size("20mb.txt", filesize=20 * 1024 * 1024)
        with open(filename, "rb") as f:
            self.client.put_object(Bucket=self.bucket_name, Key="20mb.txt", Body=f)
        self.addCleanup(self.delete_object, "20mb.txt")

        download_path = os.path.join(self.files.rootdir, "downloaded.txt")
        transfer.download_file(
            self.bucket_name, "20mb.txt", download_path, callback=progress_callback
        )
    ...
    def test_download_below_threshold(self):
        ...
        filename = self.files.create_file_with_size("foo.txt", filesize=1024 * 1024)
        with open(filename, "rb") as f:
            self.client.put_object(Bucket=self.bucket_name, Key="foo.txt", Body=f)
            self.addCleanup(self.delete_object, "foo.txt")
    ...
    def test_download_above_threshold(self):
        ...
        filename = self.files.create_file_with_size("foo.txt", filesize=20 * 1024 * 1024)
        with open(filename, "rb") as f:
            self.client.put_object(Bucket=self.bucket_name, Key="foo.txt", Body=f)
            self.addCleanup(self.delete_object, "foo.txt")
    ...
    def test_download_file_with_directory_not_exist(self):
        transfer = self.create_s3_transfer()
        self.client.put_object(Bucket=self.bucket_name, Key="foo.txt", Body=b"foo")
        self.addCleanup(self.delete_object, "foo.txt")
        download_path = os.path.join(self.files.rootdir, "a", "b", "c", "downloaded.txt")
        self.wait_until_object_exists("foo.txt")
    ...
    def test_transfer_methods_through_client(self):
        ...
        filename = self.files.create_file_with_size("foo.txt", filesize=1024 * 1024)
        self.client.upload_file(Filename=filename, Bucket=self.bucket_name, Key="foo.txt")
        self.addCleanup(self.delete_object, "foo.txt")

        download_path = os.path.join(self.files.rootdir, "downloaded.txt")
        self.wait_until_object_exists("foo.txt")
        self.client.download_file(Bucket=self.bucket_name, Key="foo.txt", Filename=download_path)
        assert_files_equal(filename, download_path)
    ...
    def test_transfer_methods_do_not_use_threads(self):
        ...
        self.client.upload_file(Bucket=self.bucket_name, Key=key, Filename=filename, Config=config)
