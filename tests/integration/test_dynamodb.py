...
class BaseDynamoDBTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.table = cls.dynamodb.create_table(
            ...
            AttributeDefinitions=[{"AttributeName": "MyHashKey", "AttributeType": "S"}],
        )
...
class TestDynamoDBTypes(BaseDynamoDBTest):
    def test_put_get_item(self):
        ...
        response = self.table.get_item(Key={"MyHashKey": "mykey"}, ConsistentRead=True)
...
class TestDynamoDBConditions(BaseDynamoDBTest):
    ...
    def scan(self, filter_expression):
        return self.table.scan(FilterExpression=filter_expression, ConsistentRead=True)
    ...
    def test_condition_between(self):
        r = self.scan(filter_expression=Attr("MyNumber").between(Decimal("1.24"), Decimal("1.26")))
        self.assertTrue(r["Items"][0]["MyNumber"] > Decimal("1.24"))
        self.assertTrue(r["Items"][0]["MyNumber"] < Decimal("1.26"))
    ...
    def test_condition_in(self):
        r = self.scan(filter_expression=Attr("MyHashKey").is_in(["notmykey", "mykey"]))
    ...
    def test_condition_size(self):
        r = self.scan(filter_expression=Attr("MyString").size().eq(len("mystring")))
    ...
    def test_condition_and(self):
        r = self.scan(
            filter_expression=(Attr("MyHashKey").eq("mykey") & Attr("MyString").eq("mystring"))
        )
        item = r["Items"][0]
        self.assertTrue(item["MyHashKey"] == "mykey" and item["MyString"] == "mystring")

    def test_condition_or(self):
        r = self.scan(
            filter_expression=(Attr("MyHashKey").eq("mykey2") | Attr("MyString").eq("mystring"))
        )
        item = r["Items"][0]
        self.assertTrue(item["MyHashKey"] == "mykey2" or item["MyString"] == "mystring")
